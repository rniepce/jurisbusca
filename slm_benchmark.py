"""
SLM Benchmark — Comparação automatizada: Pipeline SLM Local vs GPT-5.3.
...
def benchmark_gpt53(processo_text: str) -> dict:
    """Executa GPT-5.3 (chat simples) e mede métricas."""
    try:
...
def generate_report(results: list, output_path: str):
    ...
    report = ["# 📊 Benchmark: SLM Pipeline Local vs GPT-5.3\n"]
    ...
    for i, r in enumerate(results):
        for engine_result in [r.get("slm", {}), r.get("gpt53", {})]:
    ...
    gpt_latencies = [r["gpt53"]["latency_total"] for r in results if "gpt53" in r and "latency_total" in r["gpt53"] and not r["gpt53"].get("error")]

    if slm_latencies:
        report.append(f"\n**Latência média SLM Local:** {sum(slm_latencies)/len(slm_latencies):.1f}s\n")
    if gpt_latencies:
        report.append(f"**Latência média GPT-5.3:** {sum(gpt_latencies)/len(gpt_latencies):.1f}s\n")
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark SLM Pipeline vs GPT-5.3")
    ...
    parser.add_argument("--skip-gpt", action="store_true", help="Pular teste GPT-5.3 (só testar SLM)")
    ...
    for f in test_files:
        ...
        if not args.skip_gpt:
            print("  ☁️ Executando GPT-5.3...")
            result["gpt53"] = benchmark_gpt53(processo_text)
            if result["gpt53"].get("error"):
                print(f"  ⚠️ GPT-5.3: {result['gpt53']['error'][:100]}")
            else:
                print(f"  ✅ GPT-5.3: {result['gpt53']['latency_total']}s")
